 ...
 * real browser: encode BCBP text to a genuine PDF417 PNG (zxing-wasm/writer) -> draw it
 ...
 */
(window as unknown as Record<string, unknown>).__runBarcodePipelineTest = async (bcbpText: string) => {
  const written = await writeBarcodeToImageFile(bcbpText, { format: "PDF417", width: 480, height: 160, margin: 10 });
